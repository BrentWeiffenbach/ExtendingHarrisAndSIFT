...
def _load_volume(shape: str | None, modelnet_index: int | None) -> tuple[np.ndarray, str]:
...
def _plot_original_volume(volume: np.ndarray, title: str) -> None:
    ...
    fig.suptitle(f"Step 1 — Original Volume: '{title}'  shape={volume.shape}", fontsize=11)
...
def _plot_gaussian_pyramid(result: SIFT3DGaussianResult) -> None:
    ...
    fig.suptitle("Step 2 — Gaussian Pyramid  (middle Z slice per octave × scale)", fontsize=11)
...
def _plot_extrema_responses(result: SIFT3DGaussianResult, title: str) -> None:
    ...
    if occupied.shape[0] > 0:
        ...
        ax3d.scatter(
            occ[:, 2], occ[:, 1], occ[:, 0],
            c="steelblue", s=2, alpha=0.15, label="voxels",
        )
    ...
    if eg.shape[0] > 0:
        ...
        sc = ax3d.scatter(
            x_kp, y_kp, z_kp,
            c=responses, s=sizes, cmap="hot",
            vmin=0, vmax=vmax,
            edgecolors="red", linewidths=0.4,
            alpha=0.9, zorder=5, label="extrema",
        )
    ...
    if counts_per_octave:
        ...
        ax_r = ax_bar.twinx()
        ax_r.plot(x_pos, means, "D--", color="firebrick", markersize=6, label="mean |resp|")
        ...
        ax_bar.text(0.5, 0.5, "No extrema detected", ha="center", va="center",
...
def _plot_response_analysis(result: SIFT3DGaussianResult, title: str) -> None:
    ...
    axes[0].axvline(float(np.median(responses)), color="red", linestyle="--", label="median")
...
def run_walkthrough(shape: str | None = None, modelnet_index: int | None = None) -> None:
    ...
    params = SIFT3DParams(
        ...
        min_size=8,
        downsample_factor=2,
        extrema_contrast_threshold=0.005,
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="3D SIFT voxel step-by-step walkthrough")
    parser.add_argument(
        "--shape",
        type=str,
        default=None,
